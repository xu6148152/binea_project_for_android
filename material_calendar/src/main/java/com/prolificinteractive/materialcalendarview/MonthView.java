 ...
 */
@SuppressLint("ViewConstructor") public class MonthView extends ViewGroup
    ...
    @ShowOtherDates private int showOtherDates = SHOW_DEFAULTS;
    ...
    private void initView() {
        ...
        CalendarDay min = CalendarDay.from(today.getYear() - 200, today.getMonth(), today.getDay());

        CalendarDay max = CalendarDay.from(today.getYear() + 200, today.getMonth(), today.getDay());

        rangeIndex = new DateRangeIndex(min, max);
        currentIndex = rangeIndex.getCount() / 2;

        setClipChildren(false);
        setClipToPadding(false);

        month = rangeIndex.getItem(currentIndex);
    ...
    private void updateDayView() {
        ...
        for (int i = 0; i < DEFAULT_DAYS_IN_WEEK; i++) {
            WeekDayView weekDayView =
                    new WeekDayView(getContext(), CalendarUtils.getDayOfWeek(calendar));
    ...
    public void updateUi(CalendarDay month) {
        int outMonth = month.getMonth();
    }
        ...
        if (v instanceof DayView) {
            ...
            if (mcv != null) {
                ...
                for (DecoratorResult result : decoratorResults) {
                    if (result.decorator.shouldDecorate(dayView.getDate())) {
                        if (listener != null) {
                            listener.onDateClick(dayView.getDate());
                        }
                    }
                }
            }
     ...
     */
         ...
         * {@inheritDoc}
         */
        @Override protected void onMeasure ( final int widthMeasureSpec,
        final int heightMeasureSpec){
            ...
            if (specHeightMode == MeasureSpec.UNSPECIFIED
                ...
                throw new IllegalStateException(
                        "MonthView should never be left to decide it's size");
            }
        ...
        @Override public void onInitializeAccessibilityNodeInfo (@NonNull AccessibilityNodeInfo
        info){
