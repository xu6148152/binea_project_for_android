package com.zepp.www.stickyheaderrecyclerview;
...
@SuppressWarnings("unused") public class SectionAdapter
    ...
    /**
     * Called to update the ghost header for a particular section. Note, most implementations will
     * not need to ever touch the ghost header.
     *
     * @param viewHolder the view holder to update
    ...
    public int getSectionForAdapterPosition(int position) {
        ...
        for (Section section : this.sections) {
            if (position >= section.adapterPosition
                    && position < section.adapterPosition + section.length) {
                return sectionIndex;
            }
            sectionIndex++;
        }
        ...
        throw new IndexOutOfBoundsException(
                "adapterPosition " + position + " does not correspond to items in adapter");
    }

    /**
     * Given a sectionIndex and an adapter position get the local position of an item relative to
     * the sectionIndex,
    ...
    public int getPositionOfItemInSection(int sectionIndex, int adapterPosition) {
        ...
        if (localPosition > section.length) {
            throw new IndexOutOfBoundsException("adapterPosition: "
                                                        + adapterPosition
                                                        + " is beyond sectionIndex: "
                                                        + sectionIndex
                                                        + " length: "
                                                        + section.length);
        }
    ...
    /**
     * Given a sectionIndex index, and an offset into the sectionIndex where 0 is the header, 1 is
     * the ghostHeader, 2 is the first item in the sectionIndex, return the corresponding "global"
     * adapter position
     *
     * @param sectionIndex a sectionIndex index
     * @param offsetIntoSection offset into sectionIndex where 0 is the header, 1 is the first
     * item,
     * etc
     ...
     *
     * @param sectionIndex the index of the section
     * @param offsetIntoSection the offset of the item in the section where 0 would be the first
     * item in the section
    ...
    public int getAdapterPositionForSectionItem(int sectionIndex, int offsetIntoSection) {
        if (doesSectionHaveHeader(sectionIndex)) {
            return getAdapterPosition(sectionIndex, offsetIntoSection)
                    + 2; // header is at position 0, ghostHeader at position 1
        } else {
            return getAdapterPosition(sectionIndex, offsetIntoSection);
        }
    }
    ...
    /**
     * Notify that all the items in a particular section are invalid and that section should be
     * reloaded
     * Never directly call notifyDataSetChanged.
     *
     ...
     * @param itemIndex the index of the item relative to the section (where 0 is the first item in
     * the section)
     */
    public void notifySectionItemChanged(int sectionIndex, int itemIndex) {
        if (sections == null) {
            ...
            if (itemIndex >= section.numberOfItems) {
                throw new IndexOutOfBoundsException("itemIndex adapterPosition: "
                                                            + itemIndex
     ...
     *
     * @param sectionIndex index of the section
     * @param itemIndex index of the item in the section where 0 is the first position in the
     * section
     */
    ...
    @Override public int getItemViewType(int adapterPosition) {
        ...
        if (adapterPosition < 0) {
            ...
            throw new IndexOutOfBoundsException(
                    "adapterPosition cannot be > getItemCount() (" + getItemCount() + ")");
        }
    ...
    @Override public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        throw new IndexOutOfBoundsException("unrecognized viewType: "
                                                    + viewType
                                                    + " does not correspond to TYPE_ITEM, TYPE_HEADER or TYPE_FOOTER");
    }
        ...
        switch (holder.getItemViewType()) {
            ...
            default:
                throw new IllegalArgumentException("unrecognized viewType: "
                                                           ...
                                                           + " does not correspond to TYPE_ITEM, TYPE_HEADER, TYPE_GHOST_HEADER or TYPE_FOOTER");
        }
    }

    /**
     * Tag the itemView of the view holder with information needed for the layout to do its sticky
     * positioning.
     * Specifically, it tags R.id.sectioning_adapter_tag_key_view_type to the item type,
     * R.id.sectioning_adapter_tag_key_view_section
     * to the item's section, and R.id.sectioning_adapter_tag_key_view_adapter_position which is
     * the
     * adapter position of the view
     *
