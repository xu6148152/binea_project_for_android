 ...
 * StickyHeaderLayoutManager MUST be used in conjunction with SectionAdapter.
 *
 * @see SectionAdapter
 */
@SuppressWarnings("unused") public class StickyHeaderLayoutManager
    ...
    public void onAttachedToWindow(RecyclerView view) {
        ...
        try {
            adapter = (SectionAdapter) view.getAdapter();
        } catch (ClassCastException e) {
            Log.e(TAG, "onAttachedToWindow: StickyHeaderLayoutManager must be used with a RecyclerView who's adapter is a kind of SectionAdapter");
        }
    }
    ...
    View createSectionHeaderIfNeeded(RecyclerView.Recycler recycler, int sectionIndex) {
        ...
        if (adapter.doesSectionHaveHeader(sectionIndex)) {
            ...
            for (int i = 0, n = getChildCount(); i < n; i++) {
                ...
                if (getViewType(view) == SectionAdapter.TYPE_HEADER && getViewSectionIndex(view) == sectionIndex) {
                    return view;
                }
            }
        ...
        if (dy < 0) {
            ...
            while (scrolled > dy) {
                ...
                if (firstViewAdapterPosition > 0 && scrolled > dy) {
                    ...
                    boolean isHeader = itemViewType == SectionAdapter.TYPE_HEADER;
                    ...
                    int top;
                    boolean isGhostHeader = itemViewType == SectionAdapter.TYPE_GHOST_HEADER;
                    if (isGhostHeader) {
    ...
    public void recycleViewsOutOfBounds(RecyclerView.Recycler recycler) {
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            if (getViewType(view) != SectionAdapter.TYPE_HEADER) {
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            if (getViewType(view) == SectionAdapter.TYPE_HEADER && !remainingSections.contains(sectionIndex)) {
    ...
    View getTopmostChildView() {
        ...
        for (int i = 0, e = getChildCount(); i < e; i++) {
            ...
            // ignore headers
            if (getViewType(v) == SectionAdapter.TYPE_HEADER) {
                continue;
            }
    ...
    SectionAdapter.ViewHolder getViewViewHolder(View view) {
        return (SectionAdapter.ViewHolder) view.getTag(R.id.sectioning_adapter_tag_key_view_viewholder);
    }
