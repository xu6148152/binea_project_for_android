...
public class DotsView extends View {
    ...
    float mMaxOuterDotsRadius;
    float mMaxInnerDotsRadius;
    ...
    @Override protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mMaxOuterDotsRadius = w / 2 - mMaxDotsSize * 2;
    ...
    private void drawInnerDotsFrame(Canvas canvas) {
        for (int i = 0; i < DOTS_COUNT; i++) {
            int cX = (int) (mCenterX + currentRadius2 * Math.cos(
                    i * OUTER_DOTS_POSITION_ANGLE * Math.PI / 180));
            int cY = (int) (mCenterY + currentRadius2 * Math.sin(
                    i * OUTER_DOTS_POSITION_ANGLE * Math.PI / 180));
            canvas.drawCircle(cX, cY, currentDotSize2, mCirclePaints[i % mCirclePaints.length]);
        }
    }

    private void drawOuterDotsFrame(Canvas canvas) {
        for (int i = 0; i < DOTS_COUNT; i++) {
            int cX = (int) (mCenterX + currentRadius1 * Math.cos(
                    i * OUTER_DOTS_POSITION_ANGLE * Math.PI / 180));
            int cY = (int) (mCenterY + currentRadius1 * Math.sin(
                    i * OUTER_DOTS_POSITION_ANGLE * Math.PI / 180));
    ...
    private void updateDotsPaints() {
        if(currentProgress < 0.5f) {
            float progress =
                    (float) Utils.mapValueFromRangeToRange(currentProgress, 0f, 0.5f, 0, 1f);
    ...
    private void updateOuterDotsPosition() {
        if (currentProgress < 0.3f) {
            this.currentRadius1 =
                    (float) Utils.mapValueFromRangeToRange(currentProgress, 0, 0.3f, 0.0f, mMaxInnerDotsRadius * 0.8f);
        }else {
            this.currentRadius1 =
                    (float) Utils.mapValueFromRangeToRange(currentProgress, 0.3f, 1f, 0.8f * mMaxOuterDotsRadius, mMaxOuterDotsRadius);
        }

        if(currentProgress < 0.7) {
            this.currentDotSize1 = mMaxDotsSize;
        }else {
            this.currentDotSize1 =
                    (float) Utils.mapValueFromRangeToRange(currentProgress, 0.7f, 1f, mMaxDotsSize, 0);
        }
    }
    ...
    private void updateInnerDotsPosition() {
        if (currentProgress < 0.3f) {
            this.currentRadius2 =
                    (float) Utils.mapValueFromRangeToRange(currentProgress, 0, 0.3f, 0.0f, mMaxInnerDotsRadius);
        }else {
            this.currentRadius2 = mMaxInnerDotsRadius;
        }

        if(currentProgress < 0.2f) {
            this.currentDotSize2 = mMaxDotsSize;
        } else if(currentProgress < 0.5f) {
            this.currentDotSize2 =
                    (float) Utils.mapValueFromRangeToRange(currentProgress, 0.2f, 0.5f, mMaxDotsSize, 0.3 * mMaxDotsSize);

        } else {
            this.currentDotSize2 =
                    (float) Utils.mapValueFromRangeToRange(currentProgress, 0.5f, 1f, mMaxDotsSize * 0.3f, 0);
        }
    }
