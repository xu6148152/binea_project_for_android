...
import com.zepp.www.likeanimation.leonids.initializers.AccelerationInitializer;
import com.zepp.www.likeanimation.leonids.initializers.ParticleInitializer;
import com.zepp.www.likeanimation.leonids.initializers.RotationInitiazer;
import com.zepp.www.likeanimation.leonids.initializers.RotationSpeedInitializer;
import com.zepp.www.likeanimation.leonids.initializers.ScaleInitializer;
import com.zepp.www.likeanimation.leonids.initializers.SpeeddByComponentsInitializer;
import com.zepp.www.likeanimation.leonids.initializers.SpeeddModuleAndRangeInitializer;
import com.zepp.www.likeanimation.leonids.modifiers.AlphaModifier;
import com.zepp.www.likeanimation.leonids.modifiers.ParticleModifier;
...
public class ParticleSystem {
     ...
     */
    public ParticleSystem(Activity a, int maxParticles, AnimationDrawable animation,
            long timeToLive, int parentViewId) {
        this(a, maxParticles, timeToLive, parentViewId);
        // Create the particles
        for (int i = 0; i < mMaxParticles; i++) {
            mParticles.add(new AnimatedParticle(animation));
    ...
    public ParticleSystem setSpeedRange(float speedMin, float speedMax) {
        mInitializers.add(
                new SpeeddModuleAndRangeInitializer(dpToPx(speedMin), dpToPx(speedMax), 0, 360));
        return this;
    }

    public ParticleSystem setSpeedModuleAndAngleRange(float speedMin, float speedMax, int minAngle,
            int maxAngle) {
        mInitializers.add(
                new SpeeddModuleAndRangeInitializer(dpToPx(speedMin), dpToPx(speedMax), minAngle,
                        maxAngle));
        return this;
    }

    public ParticleSystem setSpeedByComponentsRange(float speedMinX, float speedMaxX,
            float speedMinY, float speedMaxY) {
    ...
    public ParticleSystem setAccelerationModuleAndAndAngleRange(float minAcceleration,
            float maxAcceleration, int minAngle, int maxAngle) {
        mInitializers.add(
                new AccelerationInitializer(dpToPx(minAcceleration), dpToPx(maxAcceleration),
                        minAngle, maxAngle));
        return this;
    }
    ...
    /**
     * Starts emiting particles from a specific view. If at some point the number goes over the
    ...
    private void onUpdate(long miliseconds) {
        while (((mEmitingTime > 0 && miliseconds < mEmitingTime) || mEmitingTime == -1) &&
                ...
                mActivatedParticles
                        < mParticlesPerMilisecond
                        * miliseconds) { // and we are under the number of particles that should be launched
