...
@SuppressLint("NewApi") public class MonkeyPatcher {

    /**
     * 用realApplication替换原有的application
     */
    public static void monkeyPatchApplication(
            Context context, Application bootstrap, Application realApplication, String externalResourceFile) {
        try {
    ...
    public static Object getActivityThread(
            Context context, Class<?> activityThread) {
        try {
            ...
            if (Build.VERSION.SDK_INT < 16) {
                ...
                if ("mColorStateListCache".equals(fieldName)) {
                    ...
                    if (type.isAssignableFrom(ArrayMap.class)) {
                        Method clearArrayMap = Resources.class.getDeclaredMethod("clearDrawableCachesLocked", new Class[] {
                                ArrayMap.class, Integer.TYPE
                        });

                        clearArrayMap.setAccessible(true);
                        clearArrayMap.invoke(resources, new Object[] {
                                cache, Integer.valueOf(-1)
                        });
                        return true;
                    }
                    if (type.isAssignableFrom(LongSparseArray.class)) {
                        Method clearSparseMap = Resources.class.getDeclaredMethod("clearDrawableCachesLocked", new Class[] {
